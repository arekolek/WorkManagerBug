...
class TimestampRvWidgetProvider : AppWidgetProvider() {

    companion object {
        private const val TAG = "TimestampRvWidget"
        private const val API_URL = "https://www.google.com/generate_204"

        fun updateAllWidgets(context: Context) {
            val appWidgetManager = AppWidgetManager.getInstance(context)
            val ids = appWidgetManager.getAppWidgetIds(
                ComponentName(context, TimestampRvWidgetProvider::class.java)
            )
            if (ids.isNotEmpty()) {
                TimestampRvWidgetProvider().onUpdate(context, appWidgetManager, ids)
            }
        }
    }
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        for (appWidgetId in appWidgetIds) {
            val views = RemoteViews(context.packageName, R.layout.widget_timestamp_rv)
            views.setTextViewText(R.id.tv_local_time, "Local: $localTime")
            views.setTextViewText(R.id.tv_result, result)

            val refreshIntent = Intent(context, TimestampRvWidgetProvider::class.java).apply {
