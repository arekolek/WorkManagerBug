...
class UpdateWidgetReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        try {
            ...
            Log.d(TAG, "Alarm fired for $target — " +
            ...
            runBlocking {
                for (attempt in 1..30) {
                    ...
                    if (success) {
                        Log.d(TAG, "Alarm — network available after $attempt attempt(s), updating $target")
                        when (target) {
                            TARGET_GLANCE -> TimestampWidget().updateAll(context)
                            TARGET_RV -> TimestampRvWidgetProvider.updateAllWidgets(context)
                        }
                        return@runBlocking
                    }
    ...
    companion object {
        ...
        fun schedule(context: Context, target: String) {
            Log.d(TAG, "Scheduling alarm ($target) in 60s")
            val intent = Intent(context, UpdateWidgetReceiver::class.java)
                .putExtra(EXTRA_TARGET, target)
            val requestCode = if (target == TARGET_GLANCE) 1 else 2
